...
namespace LiveTubeReport.View {
	public partial class OptionForm : Form {
		...
		private void button1_Click(object sender, EventArgs e) {
			...
			OpenFileDialog.Filter = "実行ファイル(*.exe;*.bat)|*.exe;*.bat";
			...
			OpenFileDialog.FileOk += (object s, System.ComponentModel.CancelEventArgs cancel) => {
				...
				if (!Path.GetExtension(dialog.FileName).Equals(".exe") && !Path.GetExtension(dialog.FileName).Equals(".bat")) {
					MessageBox.Show("実行ファイル(*.exe;*.bat)を選択してください。");
					cancel.Cancel = true;
				}
			};
